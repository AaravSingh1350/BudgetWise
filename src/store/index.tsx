...
const initializeStore = () => {
  return createStore<StoreState>((set, get) => ({
    ...
    addMultipleExpenses: (newExpenses) => {
      const expensesWithIds = newExpenses.map(exp => ({ ...exp, id: generateId() }));
      set((state) => ({ expenses: [...expensesWithIds, ...state.expenses]}));
    },
    ...
    resetData: () => {
      ...
      set({ ...getDefaultInitialState(), isLoading: false });
    },
  }));
};
...
export const useStore = () => {
  ...
  const zustandStore = useZustandStore(store);
  return zustandStore;
};
